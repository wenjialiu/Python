...
extension AddUsernameController: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        guard let indicator = scrollView.subviews.last as? UIImageView else { return }
        ...
        let cell = tblUsernames.visibleCells.first
        guard let prefix = cell?.textLabel?.text else { return }
        ...
        btnIndicator.center.y = indicator.center.y / 30.5 + 135
    ...
    func panGestureIndicator(_ pan: UIPanGestureRecognizer) {
        if pan.state == .began {
            ...
            if btnIndicator.center.y >= screenHeight - 22 {
                tblUsernames.scrollToRow(at: IndexPath(row: testArray.count - 1, section: 0), at: .bottom, animated: true)
            }
        } else {
            ...
            var point_y: CGFloat = loc_y * 29.11885 - 3901.53788
